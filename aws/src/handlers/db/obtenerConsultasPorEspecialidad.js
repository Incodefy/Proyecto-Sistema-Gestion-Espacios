...
module.exports.handler = async (event) => {
  const logger = Logger.fromEvent(event).child({ handler: 'obtenerConsultasPorEspecialidad' });
  ...
  // Rate limiting: 50 scans por minuto (scan completo, muy costoso)
  if (userSub) {
    const rateLimitCheck = await checkRateLimit(userSub, 50, 60, 'obtener-consultas-especialidad');
